...
function getSales(n, startYear, keys) {
...
function draw(type) {
  ...
  var keys = ["pc", "phone", "software"];
  var data = getSales(20, 2005, keys);
  var stack = d3.stack().keys(keys).value(function (d, key) {
    return d[key];
  }).order(d3.stackOrderNone).offset(d3.stackOffsetNone);
  var stackData = stack(data).map(function (d) {
    d.forEach(function (v) {
      return v.key = d.key;
    });
    return d;
  });
  var color = d3.scaleOrdinal().domain(keys).range(d3.quantize(d3.interpolateRainbow, keys.length + 1)).unknown("#ccc");
  var margin = {
    left: 50,
    top: 30,
    right: 30,
  ...
  var xScale = d3.scaleBand().domain(data.map(function (t) {
    return t.year;
  })).range([margin.left, width - margin.right - margin.left]); // .padding(0.05)

  var yScale = d3.scaleLinear().domain([0, d3.max(stackData, function (d) {
    return d3.max(d, function (d2) {
      return d2[1];
    });
  })]).range([height - margin.botttom - margin.top, margin.top]);
  var svg = d3.create("svg");
  svg.append("g").attr("transform", "translate(0," + yScale(0) + ")").call(d3.axisBottom(xScale).ticks(10));
  svg.append("g").attr("transform", "translate(" + margin.left + ",0)").call(d3.axisLeft(yScale)); // axis end

  if (type === "rect") {
    svg.append("g").selectAll("g").data(stackData).join("g").attr("fill", function (d) {
      return color(d.key);
    ...
    var area_1 = d3.area().x(function (d) {
  ...
  svg.attr("width", width).attr("height", height).style("border", "1px dashed").attr("viewBox", function () {
    document.body.append(this); //   const { x, y, width, height } = (this as SVGAElement).getBBox()
...
},{}]},{},["../node_modules/parcel/src/builtins/hmr-runtime.js","stack.ts"], null)
//# sourceMappingURL=/stack.5b6c903f.js.map