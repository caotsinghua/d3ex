...
function testSymbol() {
  ...
  var symbol = d3.symbol().size(function (d) {
    return d.size;
  }).type(function (d) {
    return d.type;
  });
  svg.selectAll().data(dataset).enter().append("path").attr("d", function (d) {
    return symbol(d);
...
function testChord() {
  ...
  var path = ribbon(chordData); // console.log(ribbon(chordData))

  svg.append("path").attr("d", path).attr("stroke", "#000").attr("fill", "yellow").attr("stroke-width", 2).attr("transform", "translate(100,100)");
} // testChord();


function testChord2() {
  var matrix = [[11975, 5871, 8916, 2868], [1951, 10048, 2060, 6171], [8010, 16145, 8090, 8045], [1013, 990, 940, 6907]];
  var chord = d3.chord().padAngle(Math.PI * 0.01);
  var ribbonData = chord(matrix);
  var ribbon = d3.ribbon().radius(180);
  console.log(ribbonData);
  var g = svg.append("g").attr("transform", "translate(200,200)");
  g.selectAll("path").data(ribbonData).enter().append("path").attr("d", function (d) {
    return ribbon(d);
  }).attr("fill", function (_, i) {
    var rgb = d3.color(d3.schemeCategory10[i % 10]).rgb();
...
},{}]},{},["../node_modules/parcel/src/builtins/hmr-runtime.js","symbols.ts"], null)
//# sourceMappingURL=/symbols.45407a8b.js.map