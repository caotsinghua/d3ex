...
function buildPie() {
  return __awaiter(this, void 0, void 0, function () {
    var data, pie, pieData, outR, inR, arc, arcs;
    return __generator(this, function (_a) {
      switch (_a.label) {
        ...
        case 1:
          data = _a.sent(); //   console.log(data)

          console.log(data);
          pie = d3.pie().value(function (d) {
            return d["2018年"];
          }).startAngle(Math.PI * 0.5).endAngle(Math.PI * 2.5);
          pieData = pie(data);
          console.log(pieData);
          outR = width / 3.5;
          inR = 0;
          arc = d3.arc().innerRadius(inR).outerRadius(outR);
          arcs = pieSvg.selectAll("g").data(pieData).enter().append("g").attr("transform", "translate(" + width / 2 + "," + width / 2 + ")"); // add arcs
          ...
          arcs.append("text").text(function (d) {
            return d.data["2018年"];
          }).attr("fill", "#000").attr("text-anchor", "middle").attr("transform", function (d, i) {
            var position = arc.centroid(d);
            return "translate(" + position[0] + "," + position[1] + ")";
          }).attr("dy", "0.5rem"); // add out
...
function getData() {
  return new Promise(function (resolve, reject) {
    d3.blob("./分省年度数据.csv").then(function (data) {
      var reader = new FileReader();
      reader.readAsText(data, "gb2312");

      reader.onload = function (e) {
        var res = e.target.result;
        resolve(d3.csvParse(res));
      };
    });
  });
}
...
},{}]},{},["../node_modules/parcel/src/builtins/hmr-runtime.js","layout.ts"], null)
