...
function domInit() {
  svg = svg || d3.select("body").append("svg").attr("width", width).attr("height", height).style("border", "1px dashed");
}
...
function drawChord() {
  ...
  var population = [[9000, 870, 3000, 1000, 5200], [3400, 8000, 2300, 4922, 274], [2000, 2000, 7700, 4881, 1050], [3000, 8012, 5531, 500, 400], [3540, 4310, 1500, 1900, 300]];
  var chord = d3.chord().padAngle(0.03)(population);
  var groups = chord.groups;
  console.log(chord);
  var inRadius = width / 2 * 0.7;
  var outRadius = inRadius * 1.1;
  var chordG = svg.append("g").attr("transform", "translate(" + width / 2 + "," + height / 2 + ")");
  var outG = chordG.append("g");
  var outArg = d3.arc().innerRadius(inRadius).outerRadius(outRadius);
  outG.selectAll(".outArg").data(groups).enter().append("path").classed("outArg", true).attr("d", function (d) {
    return outArg(d);
  }).attr("fill", function (_, i) {
    return d3.schemeCategory10[i % 10];
  }).attr("stroke", "#333"); // add text

  outG.selectAll(".outText").data(groups).enter().append("text").each(function (d, i) {
  ...
  var ribbon = d3.ribbon().radius(inRadius * 0.98);
  var inG = chordG.append("g");
  inG.selectAll(".inRibbon").data(chord).enter().append("path").classed("inRibbon", true).attr("d", function (d) {
    return ribbon(d);
  }).attr("fill", function (_, i) {
    return d3.schemeCategory10[i % 10];
  }).attr("stroke", "#333");
  ...
  function fade(n) {
    return function (g, i) {
      ...
      inG.selectAll(".inRibbon").filter(function (d) {
        // 没有链接到鼠标所在节点的弦
        return d.source.index !== i && d.target.index !== i;
      }).transition().style("opacity", n);
      outG.selectAll(".outArg").filter(function (_, outIndex) {
        return outIndex === i;
      }).transition().attr("fill", function (_, i) {
...
},{}]},{},["../node_modules/parcel/src/builtins/hmr-runtime.js","chord.ts"], null)
//# sourceMappingURL=/chord.01edc047.js.map