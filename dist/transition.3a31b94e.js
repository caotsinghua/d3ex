...
function test1() {
  d3.select("body").transition().duration(1000).ease(d3.easeBounce).style("background-color", d3.schemeCategory10[2]);
} // test1();
...
function testRect() {
  var rect = svg.append("rect").attr("fill", d3.schemeCategory10[4]).attr("x", 50).attr("y", 50).attr("width", 100).attr("height", 40).transition().ease(d3.easeBounce).delay(500).duration(500).attr("width", 300).transition().duration(500).attr("width", 200).transition().duration(500).attr("fill", d3.schemeCategory10[8]).transition().duration(500).attr("height", 300).transition().duration(1000).attr("width", 100).attr("height", 40).attr("fill", d3.schemeCategory10[4]);
  console.log(rect); // 返回的是一个过度对象而不是选择及对象
} // testRect();


function testAttrTween() {
  var rect = svg.append("rect").attr("fill", d3.schemeCategory10[1]).attr("x", 10).attr("y", 10).attr("width", 100).attr("height", 30);
...
function testText() {
  var rect = svg.append("rect").attr("fill", d3.schemeCategory10[1]).attr("x", 10).attr("y", 10).attr("width", 100).attr("height", 30);
  var initwidth = rect.attr("width");
  var rectTran = rect.transition().duration(3000).attrTween("width", function (d, i, a) {
    return function (t) {
      return Number(initwidth) + t * 300;
    };
  });
  var text = svg.append("text").attr("fill", "white").attr("x", 100).attr("y", 10).attr("dy", "1.2em").attr("text-anchor", "end").attr("font-size", 16).text(100);
  var initX = text.attr("x");
  var initTetx = text.text();
  var textTrans = text.transition().duration(3000).tween("text", function () {
    return function (t) {
      d3.select(this).attr("x", Number(initX) + t * 300).text(Math.floor(Number(initTetx) + t * 300));
    };
  }).transition().duration(2000).ease(d3.easeBounce).attr("font-size", 0).remove();
} // testText();


function testChildren() {
  var g = svg.append("g");
  var dataset = d3.range(3).map(function () {
    return 100;
  });
  var rect = g.selectAll("rect").data(dataset).enter().append("rect").attr("fill", function (_, i) {
    return d3.schemeCategory10[i % 10];
  }).attr("id", function (_, i) {
    return "rect" + i;
  }).attr("x", 10).attr("y", function (_, i) {
    return 10 + i * 35;
  }).attr("width", function (d) {
    return d;
  }).attr("height", 30); //   g.select("#rect1").transition().delay(500).duration(1000).attr("width", 300);
  //   g.selectAll("rect")
  //     .transition()
  //     .delay((_, i) => i * 100)
  //     .duration(1000)
...
testChildren(); // function testEach(){
// }
...
},{}]},{},["../node_modules/parcel/src/builtins/hmr-runtime.js","transition.ts"], null)
//# sourceMappingURL=/transition.3a31b94e.js.map