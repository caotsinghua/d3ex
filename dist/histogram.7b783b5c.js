...
function draw(type) {
  ...
  var dataset = getData(100);
  var xScale = d3.scaleLinear().domain(d3.extent(dataset)).nice().range([margin.left, width - margin.right]);
  var histogram = d3.histogram().domain(xScale.domain()).thresholds(xScale.ticks(20));
  var bins = histogram(dataset);
  console.log(bins, d3.max(bins, function (d) {
    return d.length;
  }));
  var yScale = d3.scaleLinear().domain([0, d3.max(bins, function (d) {
    return d.length;
  ...
  if (type === "rect") {
    gRect.selectAll("rect").data(bins).enter().append("rect").attr("width", function (d) {
    ...
    gRect.append("path").attr("d", line(bins)).attr("stroke", d3.schemeCategory10[3]).attr("fill", "none");
  }
...
function getData(n) {
  var rand = d3.randomNormal(170, 30);
...
},{}]},{},["../node_modules/parcel/src/builtins/hmr-runtime.js","histogram.ts"], null)
//# sourceMappingURL=/histogram.7b783b5c.js.map