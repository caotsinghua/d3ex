...
function d1() {
  ...
  var rects = svg.selectAll("rect").data(characters).enter().append("rect").attr("x", function (_, i) {
    return 50 + 70 * i;
  }).attr("y", 100).attr("fill", d3.schemeCategory10[3]).attr("width", 60).attr("height", 60).attr("rx", 10);
  var texts = svg.selectAll("text").data(characters).enter().append("text").text(function (d) {
    return d;
  }).attr("text-anchor", "middle").attr("x", function (_, i) {
    return 50 + 70 * i;
  }).attr("dx", 30).attr("dy", "0.5rem").attr("y", 100 + 30).attr("fill", "#fff").attr("width", 60).attr("height", 60).attr("font-size", 26);
...
function dragCircles() {
  var div = d3.select("body").append("div").style("padding", "10px").style("background", "gray");
  var svg = div.append("svg").style("background", "yellow").attr("width", width).attr("height", height);
  var circles = [{
    cx: 150,
    cy: 200,
    r: 30
  ...
  var dragaction = d3.drag().subject(function (d) {
    // return {
    //   x: d.cx,
    //   y: d.cy,
    // };
    ...
    d3.select(this).transition().attr("r", function (d) {
      return d.r * 1.3;
    }).attr("stroke", d3.schemeCategory10[1]).attr("stroke-width", 3);
  }).on("end", function () {
    console.log("拖拽结束");
    d3.select(this).transition().attr("r", function (d) {
      return d.r;
  ...
  svg.selectAll("circles").data(circles).enter().append("circle").attr("cx", function (d) {
    return d.cx;
  }).attr("cy", function (d) {
    return d.cy;
  }).attr("r", function (d) {
    return d.r;
  }).attr("fill", "black").call(dragaction);
} // dragCircles();


function useZoom() {
  var div = d3.select("body").append("div").style("padding", "10px").style("background", "gray");
  var svg = div.append("svg").style("background", "#ccc").attr("width", width).attr("height", height);
  var circles = [{
    cx: 150,
    cy: 200,
    r: 30
  ...
  var g = svg.append("g");
  var xg = svg.append("g").attr("transform", "translate(0,".concat(height - 40, ")"));
  ...
  var zoom = d3.zoom().scaleExtent([0, 5]).on("start", function () {
    ...
    console.log("zoom");
    d3.select(this).attr("transform", "translate(".concat(x, ",").concat(y, ") scale(").concat(k, ")"));
...
},{}]},{},["../node_modules/parcel/src/builtins/hmr-runtime.js","event.js"], null)
//# sourceMappingURL=/event.713d3683.js.map