...
var width = 800;
var height = 500;
var ballsDiv = d3.select("body").append("div").classed("balls", true);
...
function testInterval() {
  ...
  //   }, 2000);
  var t = d3.interval(function (nnum) {
...
function useTransition() {
  ...
  var xWidth = 300;
  var yWidth = 300;
  var xScale = d3.scaleLinear().domain([0, 1]).range([0, xWidth]);
  var yScale = d3.scaleLinear().domain([0, 1]).range([yWidth, 0]);
  var xg = svg.append("g");
  var yg = svg.append("g");
  ...
  function drawCircle(data) {
    ...
    var circleExit = circleUpdate.exit(); // update

    circleUpdate.transition().duration(500).ease(d3.easeBack).attr("cx", function (d) {
      return padding.left + xScale(d[0]);
    }).attr("cy", function (d) {
      return height - padding.bottom - yScale(d[1]);
    }); // enter

    circleEnter.append("circle") //   before trans
    .attr("cx", function (d) {
      return padding.left;
    }).attr("cy", function (d) {
      return height - padding.bottom;
    }).attr("r", 0).attr("fill", function (_, i) {
      return d3.schemeCategory10[i % 10];
    }).transition().duration(500).ease(d3.easeBounce).attr("cx", function (d) {
      return padding.left + xScale(d[0]);
    }).attr("cy", function (d) {
      return height - padding.bottom - yScale(d[1]);
    }).attr("r", 10); // exit
  ...
  function drawAxis() {
    ...
    xg.attr("transform", "translate(" + padding.left + "," + (height - padding.bottom) + ")").call(xaxis);
    yg.attr("transform", "translate(" + padding.left + "," + (height - yWidth - padding.bottom) + ")").call(yaxis);
    yScale.range([0, yWidth]);
  }

  function getData(nums) {
    return d3.range(nums).map(function () {
  ...
  generate();
  ballsDiv.append("button").text("生成").on("click", function () {
    generate();
  });
  ballsDiv.append("button").text("添加").on("click", function () {
    data.push([Math.random(), Math.random()]);
    drawAxis();
    drawCircle(data);
  });
  ballsDiv.append("button").text("清除").on("click", function () {
    data = [];
    drawCircle(data);
  });
}
...
useTransition();
var timeDiv = d3.select("body").append("div").classed("time", true);
var timeSvg = timeDiv.append("svg").classed("time", true).attr("width", width).attr("height", height);
...
function makeTime() {
  var timeText = timeSvg.append("text").attr("x", 100).attr("y", 100).attr("class", "time-text").text(getTimeStr());
...
makeTime();
var ballDiv2 = d3.select("body").append("div").attr("class", "ball-2");
var svgBall2 = ballDiv2.append("svg").attr("width", width).attr("height", height).style("border", "1px solid");

function makeBalls() {
  var t0 = Date.now();
  var t1 = Date.now();
  var dt = t1 - t0;
  var t = d3.timer(updateBall); // 相当于raf
  ...
  function updateBall() {
    ...
    var ball = svgBall2.select("circle").empty() ? svgBall2.append("circle") : svgBall2.select("circle");
...
},{}]},{},["../node_modules/parcel/src/builtins/hmr-runtime.js","timer.ts"], null)
//# sourceMappingURL=/timer.423daca9.js.map