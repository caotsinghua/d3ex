...
function drawLineChart() {
  ...
  var gdpmax = 0;
  dataset.forEach(function (_a) {
    var gdp = _a.gdp;
    gdpmax = Math.max(gdpmax, d3.max(gdp, function (d) {
      return d[1];
    }));
  });
  var xscale = d3.scaleLinear().domain([2000, 2013]).range([0, width - padding.left - padding.right]);
  var yScale = d3.scaleLinear().domain([0, gdpmax]).range([height - padding.top - padding.bottom, 0]);
  var linepath = d3.line().x(function (d) {
    return xscale(d[0]);
  }).y(function (d) {
    return yScale(d[1]);
  }).curve(d3.curveBasis);
  var colors = [d3.schemeCategory10[2], d3.schemeCategory10[8]];
  var g = svg.append("g").style("border", "1px solid #000").attr("transform", "translate(" + padding.left + "," + padding.top + ")");
  g.selectAll("path").data(dataset).enter().append("path").attr("d", function (d) {
    return linepath(d.gdp);
  }).attr("fill", "none").attr("stroke-width", 2).attr("stroke", function (_, i) {
    return colors[i];
  });
  var xg = g.append("g").attr("transform", "translate(0," + (height - padding.top - padding.bottom) + ")");
  var yg = g.append("g");
  d3.axisBottom(xscale).tickFormat(d3.format("d"))(xg);
  d3.axisLeft(yScale)(yg);
  var legends = svg.selectAll(".legends").data(dataset).enter().append("g").classed("legends", true);
  legends.append("text").text(function (d) {
    return d.country;
  }).attr("x", function (_, i) {
    return padding.left + i * 70;
  }).attr("y", padding.top - 16);
  legends.append("path").attr("d", d3.symbol().size(100).type(d3.symbolCircle)()).attr("fill", function (_, i) {
    return colors[i];
  }).attr("transform", function (_, i) {
    var x = padding.left + i * 70 - 10;
    var y = padding.top - 16 - 5;
...
function getGdp() {
  return d3.range(2000, 2014, 1).map(function (year) {
    return [year, Math.floor(10000 + Math.random() * 40000)];
  });
}
...
},{}]},{},["../node_modules/parcel/src/builtins/hmr-runtime.js","linechart.ts"], null)
//# sourceMappingURL=/linechart.14dd39c3.js.map