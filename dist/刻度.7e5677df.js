...
function domInit() {
  svg = svg || d3.select("body").append("svg").attr("width", 800).attr("height", 500).style("border", "1px dashed");
}

function testAxisLeft() {
  domInit();
  var scale = d3.scaleLinear().domain([0, 10]).range([0, 200]);
  var axisLeft = d3.axisLeft(scale).ticks(5, "s");
  var axisRight = d3.axisRight(scale).tickValues([4, 5, 6, 7]).tickSizeInner(20).tickSizeOuter(10).tickFormat(d3.format("+0.1f"));
  var g = svg.append("g").attr("transform", "translate(40,20)");
  var g2 = svg.append("g").attr("transform", "translate(100,20)");
  axisLeft(g);
  axisRight(g2);
} // testAxisLeft();
...
function testScale() {
  domInit();
  var linear = d3.scaleLinear().domain([0, 1]).range([0, 350]);
  var pow = d3.scalePow().exponent(2).domain([0, 1]).range([0, 350]);
  var log = d3.scaleLog().base(10).domain([10, 100]).range([0, 350]);
  var x1 = d3.axisBottom(linear);
  var x2 = d3.axisBottom(pow);
  var x3 = d3.axisBottom(log);
  var g1 = svg.append("g").attr("transform", "translate(40,80)");
  var g2 = svg.append("g").attr("transform", "translate(40,120)");
  var g3 = svg.append("g").attr("transform", "translate(40,160)");
  x1(g1);
  x2(g2);
  x3(g3);
} // testScale()
...
function testGraph(n) {
  var lenData = d3.range(n);
  var dataset = lenData.map(function () {
    return 50 + Math.random() * 100 | 0;
  });
  console.log(dataset);
  domInit();
  var graphHeight = 300;
  var graphWidth = 600;
  var graphG = svg.append("g").attr("width", graphWidth).attr("height", graphHeight).attr("transform", "translate(40,40)").style("border", "1px solid #000"); //   step 50,width 40,padding 10

  var hPadding = 10;
  var vPadding = 10;
  var step = 50;
  var width = 40;
  var max = d3.max(dataset); // 坐标轴

  var xScale = d3.scaleBand().domain(lenData).range([0, graphWidth]).paddingInner(0.3);
  var yScale = d3.scaleLinear().domain([0, max]).range([graphHeight, 0]);
  var xAxis = d3.axisBottom(xScale);
  var yAxis = d3.axisLeft(yScale).ticks(10);
  var xg = svg.append("g").attr("transform", "translate(40,".concat(graphHeight + 40, ")"));
  var yg = svg.append("g").attr("transform", "translate(40,40)");
  xAxis(xg);
  yAxis(yg);
  graphG.selectAll("rect").data(dataset).enter().append("rect").attr("x", function (_, i) {
  ...
  graphG.selectAll("text").data(dataset).enter().append("text").attr("x", function (_, i) {
    return xScale.paddingOuter() + i * xScale.step(); // 起点值
  }).attr("y", function (d, i) {
    return yScale(d);
  }).attr("text-anchor", "middle").attr("dx", function (_, i) {
...
},{}]},{},["../node_modules/parcel/src/builtins/hmr-runtime.js","刻度.js"], null)
//# sourceMappingURL=/刻度.7e5677df.js.map