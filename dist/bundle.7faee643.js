...
function draw() {
  ...
  var data = d3.hierarchy(cities);
  var radius = width / 2 - 50;
  var cluster = d3.cluster().size([360, radius]).separation(function (a, b) {
    return (a.parent == b.parent ? 1 : 2) / a.depth;
  });
  cluster(data);
  ...
  console.log(links);
  var bundleLine = d3.lineRadial().curve(d3.curveBundle).radius(function (d) {
    return d.y;
  }).angle(function (d) {
    return d.x * Math.PI / 180;
  });
  var gBundle = svg.append("g").attr("transform", "translate(" + width / 2 + "," + height / 2 + ")");
  gBundle.selectAll(".link").data(links).enter().append("path").attr("d", function (_a) {
    var source = _a[0],
        target = _a[1];
    return bundleLine(source.path(target));
  }).attr("stroke", "#444").attr("fill", "none");
  console.log(nodes);
  var gnodes = gBundle.selectAll(".node").data(nodes.filter(function (d) {
    return !d.children;
  })).enter().append("g").attr("transform", function (d) {
    return "rotate(" + (d.x - 90) + ") translate(" + d.y + ") rotate(" + (90 - d.x) + ")";
  });
  gnodes.append("circle").attr("r", 4).attr("fill", "blue");
  gnodes.append("text").text(function (d) {
    return d.data.name;
  });
}
...
},{}]},{},["../node_modules/parcel/src/builtins/hmr-runtime.js","bundle.ts"], null)
//# sourceMappingURL=/bundle.7faee643.js.map