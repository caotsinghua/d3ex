...
var svg = d3.select("body").append("svg").attr("width", containerWidth).attr("height", containerHeight).style("border", "1px dashed"); // 线段生成器

function testLines() {
  var lines = [[80, 80], [200, 100], [200, 200], [100, 200]];
  var linePath = d3.line();
  var svg = d3.select("body").append("svg").attr("width", 500).attr("height", 500).style("border", "1px dashed"); //   svg
  //     .append("path")
  ...
  var linep2 = d3.line().x(function (d) {
    return d;
  }).y(function (d, i) {
    return i % 2 == 0 ? 40 : 120;
  }) // .curve(d3.curveBundle.beta(0.75))
  // .curve(d3.curveCardinal)
  .curve(d3.curveStep); // .defined((d) => {
  //   return d < 200;
  // });

  svg.append("path").attr("d", linep2(lines2)).attr("stroke", "#000").attr("fill", "none");
} // testLines();
// 区域生成器
...
function testArea() {
  var height = 300;
  var nums = 20;
  var dataset = d3.range(nums).map(function () {
    return Math.floor(50 + Math.random() * 100);
  });
  var areaPath = d3.area().x(function (d, i) {
    return i * (containerWidth / (nums - 1));
  }).y0(function (d, i) {
    return containerHeight;
  }).y1(function (d, i) {
    return containerHeight - d;
  }).curve(d3.curveBasis); // .curve(d3.curveMonotoneY);

  svg.append("path").attr("d", areaPath(dataset)).attr("stroke", d3.schemeCategory10[2]).attr("stroke-width", 2).attr("fill", d3.schemeCategory10[3]);
} // testArea();
// 弧生成器
...
function testAngle() {
  ...
  var arcPath = d3.arc();
  svg.append("path").attr("d", arcPath(dataset)).attr("transform", "translate(100,200)").attr("stroke", d3.schemeCategory10[1]).attr("stroke-width", 3).attr("fill", d3.schemeCategory10[5]);
} // testAngle()
...
function testBing() {
  ...
  var arcPath = d3.arc().cornerRadius(4).padAngle(Math.PI * 0.01);
  ...
  svg.selectAll("path").data(dataset).enter().append("path").attr("transform", "translate(120,120)").attr("d", function (d) {
    return arcPath(d);
  }).attr("stroke", "none").attr("fill", function (_, i) {
    return d3.schemeCategory10[i % 10];
  }).attr("stroke-width", 2);
  svg.selectAll("text").data(dataset).enter().append("text").attr("text-anchor", "middle").attr("fill", function (_, i) {
    return d3.color(d3.schemeCategory10[i % 10]).brighter(3).toString();
  }).attr("font-size", 18).text(function (d) {
...
},{}]},{},["../node_modules/parcel/src/builtins/hmr-runtime.js","line.ts"], null)
//# sourceMappingURL=/line.aada59ad.js.map